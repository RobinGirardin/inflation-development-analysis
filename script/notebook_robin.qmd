...
Same developpement as before but by using a logarithmic transformation on the data. This transformation works great on data with heavy tails. This alternative is better than zooming in as filtering important outliers, which are integraly part of the information. In this graph, the richer the country, the lower and less stable seems inflation to be.
...
join_labeled %>% 
    ...
    ggplot(
        ...
        mapping = aes(x = year, y = log(inflation_pct))) +
            geom_point(color = "#2b8cbe", alpha = 0.3) +
            geom_smooth(
                mapping = aes(color = wealth_class)) +
                    labs(x = "Year", y = "Inflation (%)") +
                    ggtitle(" Wealth class inflation rate (2000-2024)") +
                    theme_minimal()
```
